...
import { decodePersonaFromUrl, addCustomPersona, findExistingPersona } from "@/lib/personas"
  ...
  useEffect(() => {
    ...
    const importPersona = () => {
      try {
        ...
        const existingPersona = findExistingPersona(persona)
        ...
        if (existingPersona) {
          ...
          setTimeout(() => {
            navigate(`/spaces/${existingPersona.id}`)
          }, 2000)
          return
        }
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 text-center shadow-xl">
          ...
          {status === 'exists' && (
            <>
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
                <span className="font-semibold text-green-600 dark:text-green-400">{personaName}</span> is already in your spaces
              </p>
              <div className="flex items-center justify-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                <Sparkles className="h-3.5 w-3.5" />
                <span>Redirecting to your space...</span>
              </div>
            </>
          )}
